...
yr_cols_full = [str(x) for x in range(start_year_fut, end_year_fut + 1)]
years_full = list(range(start_year_fut, end_year_fut + 1))
plot_start_idx = years_full.index(start_year_avg)
plot_end_idx = years_full.index(end_year_avg) + 1
...
# Full-period versions for production-weighted dy (weights from 1990-2019)
dy_ipcc_full = df_dy.merge(df_ipcc[["country", ar6_region]], on="country", how="left")
dp_ipcc_full = df_dp.merge(df_ipcc[["country", ar6_region]], on="country", how="left")
...
for crop in crops:
    for region in regions:
        dy_rc = dy_ipcc_full[(dy_ipcc_full['crop'] == crop) & (dy_ipcc_full[ar6_region] == region)]
        dp_rc = dp_ipcc_full[(dp_ipcc_full['crop'] == crop) & (dp_ipcc_full[ar6_region] == region)]
        country_vals = dy_avg_years[(dy_avg_years['crop'] == crop) & (dy_avg_years[ar6_region] == region)]['value']
        if len(dy_rc) == 0 or len(country_vals) == 0:
            continue
        weighted_mean = float(utils.compute_dy_weighted(dy_rc, dp_rc, yr_cols_full, prod_weights=prod_weights, weight_opt=weight_opt)[plot_start_idx:plot_end_idx].mean())
        n = len(country_vals)
        se = country_vals.std() / np.sqrt(n) if n > 1 else 0.0
        dy_stats_records.append({
            ar6_region: region, 'crop': crop,
            'median': weighted_mean,
            'q25': weighted_mean - 1.96 * se,
            'q75': weighted_mean + 1.96 * se,
            'min': country_vals.min(),
            'max': country_vals.max(),
        })
dy_stats = pd.DataFrame(dy_stats_records)
...
def plot_data(ax, stats, crop, title, label, regions, region_color_dict, bar_width, is_dp=True):
...
for col, crop in enumerate(['maize', 'wheat', 'soy']):
    ...
    # Plot dp (top row)
    ax = axes[0, col]
    plot_data(ax, dp_stats, crop, title, label, regions, region_color_dict, bar_width, is_dp=True)
    if col == 0:
    ...
    ax = axes[1, col]
    plot_data(ax, dy_stats, crop, title, label, regions, region_color_dict, bar_width, is_dp=False)
    if col == 0:
