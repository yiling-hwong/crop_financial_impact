...
for crop in crops:
    for region in regions:
        degdp_rc = degdp_ipcc[(degdp_ipcc['crop'] == crop) & (degdp_ipcc[ar6_region] == region)]
        de_rc = de_ipcc[(de_ipcc['crop'] == crop) & (de_ipcc[ar6_region] == region)]
        country_vals = degdp_avg_years[(degdp_avg_years['crop'] == crop) & (degdp_avg_years[ar6_region] == region)]['value']
        if len(degdp_rc) == 0 or len(country_vals) == 0:
            continue
        weighted_mean = float(utils.compute_degdp_weighted(degdp_rc, year_str_avg_degdp, gdp_weights=gdp_weights, weight_opt=weight_opt).mean())
        n = len(country_vals)
        se = country_vals.std() / np.sqrt(n) if n > 1 else 0.0
        degdp_stats_records.append({
...
degdp_stats = pd.DataFrame(degdp_stats_records)
