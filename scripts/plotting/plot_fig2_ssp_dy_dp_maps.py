...
ssp_diffs = [("ssp126", "ssp245"), ("ssp126", "ssp370")]  # tuples of (scenario, reference)
...
# Get mean over crops for dy
dy_long = dy_long.groupby(['country', 'ssp', 'year'])['value'].mean().reset_index()

# Get sum over crops for dp based on dp_option
...
dy_ipcc_melted = dy_ipcc.melt(id_vars=['country', 'ssp', 'esm', 'crop', 'region_ar6_dev'], var_name='year',
                              value_name='value')
dp_ipcc_melted = dp_ipcc.melt(id_vars=['country', 'ssp', 'esm', 'crop', 'region_ar6_dev'], var_name='year',
                              value_name='value')
...
gs = fig.add_gridspec(4, 2, height_ratios=[2.0, 2.0, 1.0, 1.0])
...
for i in range(2):
    ...
    if i == 0:
        big_axes[-1].set_title(f"{ssps[0].upper()}–{ssps[1].upper()}", fontsize=title_fontsize, weight="bold", y=1.35,
                               pad=5)
    else:
        big_axes[-1].set_title(f"{ssps[0].upper()}–{ssps[2].upper()}", fontsize=title_fontsize, weight="bold", y=1.35,
                               pad=5)
...
for index, (scenario, reference) in enumerate(ssp_diffs):
    ...
    if index == 0:
        ax.set_position([0.03, 0.74, 0.5, 0.2])  # [left, bottom, width, height]
    if index == 1:
        ax.set_position([0.5, 0.74, 0.5, 0.2])  # [left, bottom, width, height]
...
cax_dy = fig.add_axes([0.25, 0.72, 0.5, 0.005])  # left,bottom,width,height
...
cbar.set_label(r'$\Delta \overline{dy}$ [%]', fontsize=label_fontsize)
...
for index, (scenario, reference) in enumerate(ssp_diffs):
    ...
    if index == 0:
        ax.set_position([0.03, 0.45, 0.5, 0.2])  # [left, bottom, width, height]
    if index == 1:
        ax.set_position([0.5, 0.45, 0.5, 0.2])  # [left, bottom, width, height]

    # Add alphabet label
    ax.text(-0.03, 0.9, label_alphabets[index + 2], transform=ax.transAxes, fontsize=label_fontsize)
...
cax_dp = fig.add_axes([0.25, 0.43, 0.5, 0.005])  # left,bottom,width,height
cbar = plt.colorbar(sm, cax=cax_dp, orientation='horizontal', extend='both'
                    , format=utils.format_fn
                    # ,ticks=ticks_dp
                    )
cbar.set_label(r'$\Delta dp$ [Mt]', fontsize=label_fontsize)
cbar.ax.tick_params(labelsize=tick_fontsize)
...
for index, (scenario, reference) in enumerate(ssp_diffs):
    ...
    if index == 0:
        ax.set_position([0.1, 0.25, 0.4, 0.11])  # [left, bottom, width, height]
    if index == 1:
        ax.set_position([0.55, 0.25, 0.4, 0.11])  # [left, bottom, width, height]

    # Add alphabet label
    ax.text(0.02, 0.8, label_alphabets[index + 4], transform=ax.transAxes, fontsize=label_fontsize)
    ...
    if index == 0:
        ax.set_ylabel(r'$\Delta \overline{dy}$ [%]', fontsize=label_fontsize)
...
for index, (scenario, reference) in enumerate(ssp_diffs):
    ...
    if index == 0:
        ax.set_position([0.1, 0.1, 0.4, 0.11])  # [left, bottom, width, height]
    if index == 1:
        ax.set_position([0.55, 0.1, 0.4, 0.11])  # [left, bottom, width, height]

    # Add alphabet label
    ax.text(0.02, 0.8, label_alphabets[index + 6], transform=ax.transAxes, fontsize=label_fontsize)
    ...
    ax.set_xlabel("Year", fontsize=label_fontsize)
    if index == 0:
        ax.set_ylabel(r'$\Delta dp$ [Mt]', fontsize=label_fontsize)
    else:
        ax.set_yticklabels([])
