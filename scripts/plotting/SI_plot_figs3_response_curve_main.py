...
colors_tmax = {'maize': '#E31A1C', 'wheat': '#F28E2B', 'soy': '#E6B84C'}
colors_spei = {'maize': '#0B6E69', 'wheat': '#2A9D8F', 'soy': 'forestgreen'}
#------------------------------------------------------------------------
...
root_dir = f'../../data'
input_hist_file = f"{root_dir}/historical/linregress_inputs/{irr_str}INPUT_HISTORICAL_spei{spei_month}.csv"
...
def tmax_ci(T, T_ref, I=0.0, z=1.96):
    ...
    for t in T:
        d = np.array([
            t - T_ref, t ** 2 - T_ref ** 2, t ** 3 - T_ref ** 3,
            I * (t - T_ref), I * (t ** 2 - T_ref ** 2), I * (t ** 3 - T_ref ** 3)
        ])
        se.append(np.sqrt(d @ cov_t_full @ d))
    se = np.array(se)
...
def spei_ci(S, S_ref, I=0.0, z=1.96):
    ...
    for s in S:
        d = np.array([
            s - S_ref, s ** 2 - S_ref ** 2, s ** 3 - S_ref ** 3,
            I * (s - S_ref), I * (s ** 2 - S_ref ** 2), I * (s ** 3 - S_ref ** 3)
        ])
        se.append(np.sqrt(d @ cov_s_full @ d))
    se = np.array(se)
...
models = []
for crop in crops:
    model_file = f"{model_dir}/model_{crop}_{pred_str}_spei{spei_month}.pickle"
...
for i, crop in enumerate(crops):
    ...
    if crop_col in df.columns:
        df_crop = df[df[crop_col] == crop].copy()
    else:
        df_crop = df.copy()
    ...
    print(f"  Tmax range (1-99%): {T_p01:.1f} – {T_p99:.1f} | mean: {T_mean:.1f}")
    print(f"  SPEI range (1-99%): {S_p01:.2f} – {S_p99:.2f}  | mean: {S_mean:.2f}")
    ...
    scenario_labels = [crop.capitalize()]

    # ── Plot tmax response ─────────────────────────────────────────────
    ax    = axes[i, 0]
    color = colors_tmax[crop]

    lab = label_alphabets[i*2]

    for lbl in scenario_labels:
        yield_T = tmax_response(T_range, T_mean)
        ci_T    = tmax_ci(T_range, T_mean)
        ax.plot(T_range_C, yield_T, color=color, linewidth=2.0, linestyle="solid", label=lbl)
        ax.fill_between(T_range_C, yield_T - ci_T, yield_T + ci_T, alpha=0.1, color=color)

    ax.axhline(0, color='black', linewidth=0.8, linestyle='--', alpha=0.6)
    ax.axvline(T_mean_C, color='dimgrey', linewidth=1.8, linestyle=':', alpha=1.0, label=f'Mean ({T_mean_C:.1f}°C)')
    ...
    ax.set_title(f'{crop.capitalize()}', fontsize=title_fontsize)
    ax.text(0.0, 1.2, lab, transform=ax.transAxes, fontsize=label_fontsize + 1, fontweight='bold', va='top',ha='right')
    #ax.legend(fontsize=label_fontsize)
    ax.grid(True, alpha=0.25)

    # ── Plot SPEI response ─────────────────────────────────────────────
    ax    = axes[i, 1]
    color = colors_spei[crop]

    lab = label_alphabets[i * 2 + 1]

    for lbl in scenario_labels:
        yield_S = spei_response(S_range, S_mean)
        ci_S    = spei_ci(S_range, S_mean)
        ax.plot(S_range, yield_S, color=color, linewidth=2.0, linestyle="solid", label=lbl)
        ax.fill_between(S_range, yield_S - ci_S, yield_S + ci_S,alpha=0.1, color=color)

    ax.axhline(0, color='black', linewidth=0.8, linestyle='--', alpha=0.6)
    ax.axvline(S_mean, color='dimgrey', linewidth=1.8, linestyle=':', alpha=1.0,label=f'Mean ({S_mean:.2f})')
    ax.set_xlim(xmin_s, xmax_s)
    ax.set_ylim(ymin_s, ymax_s)
    ax.set_xlabel(f'Growing season SPEI', fontsize=label_fontsize, labelpad=10)
    ax.set_ylabel('Yield change (%)', fontsize=label_fontsize)
    ax.tick_params(axis='both', labelsize=tick_fontsize)
    ax.set_title(f'{crop.capitalize()}', fontsize=title_fontsize)
    ax.text(0.0, 1.2, lab, transform=ax.transAxes, fontsize=label_fontsize + 1, fontweight='bold', va='top', ha='right')
