...
def plot_data(ax, stats, crop, title, regions, region_color_dict, bar_width, is_de=True):
...
for col, crop in enumerate(['maize', 'wheat', 'soy']):
    ...
    # Plot dp (top row)
    ax = axes[0, col]
    plot_data(ax, de_stats, crop, title, regions, region_color_dict, bar_width, is_de=True)
    if col == 0:
        ax.set_ylabel(r'$de$ [Billion US\$]', fontsize=label_fontsize)

    # Plot dy (bottom row)
    title = f"{label_alphabets[n + 3]} {crop.capitalize()}"
    ax = axes[1, col]
    plot_data(ax, degdp_stats, crop, title, regions, region_color_dict, bar_width, is_de=False)
    if col == 0:
