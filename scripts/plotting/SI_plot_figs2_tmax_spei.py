...
labels = [r'$T_{\mathrm{max}}$ change [°C]', f"SPEI change [-]"]
var_names = {"tmax": "Surface Max. temperature",
             "spei": f"SPEI-{spei_month}"}
var_units = {"tmax": "K",
             "spei": "-"}
...
gs = fig.add_gridspec(2, 2, height_ratios=[1.8, 1.0])
...
for var in vars:
    if var == "spei":
        var_file = f"{root_dir}/historical/climate/{var}/{var}{spei_month}_mean_country_{crop}_yearly.csv"
    else:
...
for idx, var in enumerate(vars):
    ...
    ax_map.get_figure().axes[-1].set_xlabel(labels[idx], fontsize=label_fontsize)
    ax_map.get_figure().axes[-1].tick_params(labelsize=tick_fontsize)

    # Add map title
    ax_map.text(-0.03, 1.0, subplt_labels[idx], transform=ax_map.transAxes, fontweight='bold',
    ...
    ax_ts.set_ylim(ylim_min, ylim_max)
    ax_ts.tick_params(axis='both', labelsize=tick_fontsize)

    # Add time series title
    ax_ts.text(-0.03, 1.1, subplt_labels[idx + 2], transform=ax_ts.transAxes, fontweight='bold',
               fontsize=label_fontsize + 1)

    # Add y-label for all time series plots
    ax_ts.set_ylabel(labels[idx], fontsize=label_fontsize)
