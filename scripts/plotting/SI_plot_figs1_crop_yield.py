...
for idx, crop in enumerate(crops):
    ...
    plt_title = f"{crop.capitalize()}"
    label = f"{subplt_labels[idx]}"
    ...
    ax_map.text(-0.03, 1.0, label, transform=ax_map.transAxes, fontweight='bold', fontsize=label_fontsize + 1)
    ax_map.coastlines(linewidth=0.2)
    ax_map.gridlines(linestyle='--', alpha=0.5)

    # Create time series subplot
    plt_title = f"{crop.capitalize()}"
    label = f"{subplt_labels[idx + 3]}"
    ...
    ax_ts.text(-0.03, 1.1, label, transform=ax_ts.transAxes, fontweight='bold', fontsize=label_fontsize + 1)
    ...
    if idx == 1:
        ...
        cax = fig.add_axes([0.275, 0.06 + (0.24 * (3 - idx)), 0.5, 0.01])  # Positioning [left, bottom, width, height]
